...
PROJECT_DIR = "/Users/aristotle/Projects/Python-Idiomatic"
OUTPUT_DIR = os.path.join(PROJECT_DIR, "Outputs")
OUTPUT_PATH = os.path.join(OUTPUT_DIR, "changed-marked-pdf.pdf")
COVER_LOCKUP_PATH = os.path.join(PROJECT_DIR, "assets", "dewr_cover_lockup_white.png")
...
class TimeSavingsPanel(Flowable):
    ...
    def draw(self):
        ...
        max_value = 75
        rows = [
            ("M365 Copilot", 67.9, "68 (69) minutes per day", "5.7 hours per week", DEWR_DARK_GREEN),
            ("Copilot Chat", 33.5, "34 (34) minutes per day", "2.8 hours per week", DEWR_DARK_GREY),
        ]
...
class CopilotEngagementDeltaPanel(Flowable):
    ...
    def __init__(self, width):
        ...
        self.box_width = width
        self.rows = [
            ("Rated very or extremely useful", "68% (67%)", "35% (37%)"),
            ("Used at least weekly", "81% (80%)", "55% (56%)"),
            ("Used daily or most of day", "65% (63%)", "25% (27%)"),
        ]
        self._height = 122
...
class MarginalValuePanel(Flowable):
    ...
    def draw(self):
        ...
        for y, metric, chat_value, m365_value, highlight_idx in [
            (h - 43, "Added value beyond Copilot", "75.0% (73.5%)", "82.1% (81.5%)", 0),
            (h - 61, "Used weekly+", "54.5% (52.9%)", "50.0% (51.9%)", None),
        ]:
            c.setFillColor(DEWR_DARK_GREY)
    ...
    def draw(self):
        ...
        draw_group(
            ...
            [
                ...
                ("Copilot", 69.7, "69.7% (70.6%)", comparison_grey),
            ],
        )
        draw_group(
            right_x,
            "M365 Copilot",
            [
                ("Copilot", 92.9, "92.9% (92.6%)", DEWR_DARK_GREY),
...
class PriorExperienceComparisonPanel(Flowable):
    ...
    def draw(self):
        ...
        columns = [
            ("Experienced/highly experienced", "91%", "reported some or significant added value", DEWR_DARK_GREEN),
            ("Some prior experience", "73%", "reported some or significant added value", DEWR_DARK_GREY),
            ("No/basic experience", "67%", "reported some or significant added value", DEWR_DARK_GREY),
        ]
        label_style = ParagraphStyle(
...
class AllToolTaskProfilePanel(Flowable):
    ...
    def draw(self):
        ...
        axis_x = pad + label_w
        value_label_w = 104
        axis_w = w - axis_x - value_label_w - 24
        head_y = h - 22
        tick_y = h - 45
        top_y = h - 66
        bottom_y = top_y - self.row_h * (len(self.rows) - 1) - 8
        ...
        legend_w = 0
        for name, _ in self.series:
            legend_w += 5 + marker_gap + c.stringWidth(name, "Helvetica", 6.6) + item_gap
        legend_w -= item_gap
        legend_x = w - pad - legend_w
        for name, color in self.series:
            c.setFillColor(color)
            c.circle(legend_x, head_y + 2, 2.4, fill=1, stroke=0)
            c.setFillColor(DEWR_DARK_GREY)
            c.drawString(legend_x + marker_gap, head_y - 1, name)
            legend_x += 5 + marker_gap + c.stringWidth(name, "Helvetica", 6.6) + item_gap
        ...
        for r_idx, (label, values, old_values) in enumerate(self.rows):
            ...
            for i, value in enumerate(values):
                ...
                x = positions[i]
                c.setFillColor(color)
                c.circle(x, y, 3.4 if highlighted else 2.8, fill=1, stroke=0)

                if highlighted and old_values[i] is None:
                    label_text = f"{value:.1f}%"
                    c.setFont("Helvetica-Bold", 6.8)
                    text_w = c.stringWidth(label_text, "Helvetica-Bold", 6.8)
                    label_x = x + 5
                    c.setFillColor(color)
                    if label_x + text_w > axis_x + axis_w + 38:
                        c.drawRightString(x - 5, y - 2, label_text)
            ...
            value_x = axis_x + axis_w + 10
            c.setFillColor(DEWR_RED)
            c.setFont("Helvetica", 5.8)
            c.drawString(value_x, y + 3, f"M365 {values[3]:.1f}% ({old_values[3]:.1f}%)")
            c.drawString(value_x, y - 6, f"Chat {values[4]:.1f}% ({old_values[4]:.1f}%)")
...
class TaskFootprintExhibit(Flowable):
    ...
    def __init__(self, width):
        ...
        self.rows = [
            ...
            ("Research, problem solving or generating ideas", 67.7, 66.7, 43.6, 43.9, True),
    ...
    def draw(self):
        ...
        axis_x = pad + label_w
        value_label_w = 104
        axis_w = w - axis_x - value_label_w - 12
        top_y = h - 60
        row_gap = 23
        ...
        for i, (label, m365, old_m365, chat, old_chat, highlight) in enumerate(self.rows):
            ...
            c.setFont("Helvetica-Bold" if highlight else "Helvetica", 6.1)
            c.setFillColor(DEWR_RED)
            value_x = axis_x + axis_w + 10
            c.drawString(value_x, y + 2, f"M365 {m365:.1f}% ({old_m365:.1f}%)")
            c.drawString(value_x, y - 7, f"Chat {chat:.1f}% ({old_chat:.1f}%)")
...
def build_report():
    ...
    def access_evidence_table():
        ...
        data = [
            ...
            [
                ...
                Paragraph("81.8% (82.4%)", value_style_red),
                Paragraph("78.6% (77.8%)", value_style_red),
            ],
            [
                Paragraph("Copilot rated at least moderately useful", measure_style),
                Paragraph("69.7% (70.6%)", value_style_red),
                Paragraph("92.9% (92.6%)", value_style_red),
            ],
            [
                Paragraph("Public AI used weekly or more", measure_style),
                Paragraph("54.5% (52.9%)", value_style_red),
                Paragraph("50.0% (51.9%)", value_style_red),
            ],
            [
                Paragraph("Copilot used weekly or more", measure_style),
                Paragraph("57.6% (58.8%)", value_style_red),
                Paragraph("85.7% (85.2%)", value_style_red),
            ],
            [
                Paragraph("Public AI added value beyond Copilot", measure_style),
                Paragraph("75.0% (73.5%)", value_style_red),
                Paragraph("82.1% (81.5%)", value_style_red),
            ],
        ]
        first_w = width * 0.52
    ...
    story.append(ValueSignalsPanel(width, [
        ...
        ("1.9x (1.8x)", "M365 Copilot users were 1.9x (1.8x) as likely to rate Copilot very or extremely useful"),
        ("1.5x (1.4x)", "M365 Copilot users were 1.5x (1.4x) as likely to use Copilot weekly or more often"),
    ], primary_count=1))
    story.append(sp(12))
    ...
    story.append(sp(6))
    story.append(Paragraph(
        f'The reported <font color="#91040D">68 minutes per day (69 minutes per day)</font> for M365 Copilot users is in the same broad range as the '
    ...
    story.append(Paragraph(
        ...
        "existing Copilot version. Among Copilot Chat/basic users, <font color=\"#91040D\"><b>81.8% (82.4%)</b></font> rated public AI tools useful, "
        "compared with <font color=\"#91040D\"><b>69.7% (70.6%)</b></font> who rated Copilot at least moderately useful. Among M365 Copilot users, the pattern was "
        "reversed: <font color=\"#91040D\"><b>92.9% (92.6%)</b></font> rated Copilot at least moderately useful, compared with <font color=\"#91040D\"><b>78.6% (77.8%)</b></font> who rated public AI "
        "tools useful.", body))
    story.append(sp(4))
    ...
    story.append(source_note(
        ...
        "provided valid responses for the relevant measures. <font color=\"#91040D\">M365 Copilot n=28 (n=27); "
        "Copilot Chat/basic n=33 (n=34)</font> for Section 2 measures. Copilot engagement rows use their valid "
        "Section 1 populations. Useful means moderately, very or extremely useful."))
    story.append(sp(8))
    story.append(Paragraph(
        "Weekly use points to the same relative pattern. Public AI weekly use was similar across access "
        "groups (<font color=\"#91040D\"><b>54.5% (52.9%)</b></font> for Copilot Chat/basic users and <font color=\"#91040D\"><b>50.0% (51.9%)</b></font> for M365 Copilot users). However, "
        "for Copilot Chat/basic users, public AI weekly use sat close to their Copilot weekly use "
        "(<font color=\"#91040D\"><b>57.6% (58.8%)</b></font>), while for M365 Copilot users it sat well below their Copilot weekly use "
        "(<font color=\"#91040D\"><b>85.7% (85.2%)</b></font>). Public AI added value beyond Copilot for "
        "<font color=\"#91040D\"><b>75.0% (73.5%)</b></font> of Copilot Chat/basic users and "
        "<font color=\"#91040D\"><b>82.1% (81.5%)</b></font> of M365 Copilot users.", body))
    ...
    story.append(KeepTogether([
        ...
        Paragraph(
            ...
            "one comparison dimension (<b>86%</b> vs <font color=\"#91040D\"><b>71% (69%)</b></font> for lower-experience users) and to strongly "
    ...
    story.append(ValueSignalsPanel(width, [
        ...
        ("12% (11%)", "Ethical concerns encountered"),
    ...
    story.append(Paragraph(
        ...
        "<font color=\"#91040D\"><b>12% (11%)</b></font> reported ethical "
    ...
    story.append(Paragraph(
        ...
        "public tools, compared with <font color=\"#91040D\"><b>60.0% (61.9%)</b></font> among respondents who did not rate both channels "
        "effective.", body))
    story.append(sp(8))
    ...
    # Visual options
    story.append(PageBreak())
    story.append(Paragraph("Visual Options", h2))
